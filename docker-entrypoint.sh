...
set -e
cd /opt/app
printf "Starting app in port $PORT...\n"
printf "Runtype: $RUNTYPE\n"
...
if [[ "${RUNTYPE}" == "celery_worker" ]]; then
    ...
    if [[ -z "${NEW_RELIC_LICENSE_KEY}" ]]; then
      ...
      exec newrelic-admin run-program gunicorn --bind :$PORT --workers 1 --worker-class uvicorn.workers.UvicornWorker  --threads 8 main:app
    else
      printf "started Docker container as runtype \e[1;93mweb\e[0m\n"
